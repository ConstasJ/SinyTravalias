...
export const WikiReferenceSchema = z.object({
    ...
    target: z.string().describe("引用的目标，指向外部存储（文件，OSS，向量存储等）中的一个文本块或者向量块"),
});
...
export const WikiHistoryEntrySchema = z.object({
    ...
    action: z.enum(["create", "update", "delete"]).describe("历史记录的操作类型，表示这个历史记录是创建、更新还是删除了一个条目"),
    diff: z.object({
...
export const WikiNavboxSchema = z.object({
    ...
    groups: z.array(z.object({
        ...
        items: z.array(z.string()).describe("分组中的条目列表，包含这个分组中所有条目的名字"),
...
export const WikiCategorySchema = z.object({
    ...
    items: z.array(z.object({
        ...
        type: z.enum(["entry", "subcategory"]).describe("项目类型，表示这个项目是一个条目还是一个子分类"),  

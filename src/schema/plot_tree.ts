...
export const PlotNodeMetadataSchema = z.object({
    ...
    snapshot: z.object({
        ...
        patches: z
            ...
            .describe('当前情节节点的JSON Patch列表，记录从父节点到当前节点的状态机变化'),
    ...
    generation: z.object({
        ...
        params: z
            ...
            .describe(
                '生成这个情节节点的语言模型的参数，键为参数的名字，值为参数的值，可以根据需要添加任意的合法JSON数据结构'
            ),
        metrics: z.object({
    ...
    semantic: z.object({
        mainCharacters: z
            .array(z.string())
            .describe('这个情节节点的主要角色列表，包含在这个节点中出现的主要角色的名字'),
        locationId: z
            .string()
            .optional()
            .describe('这个情节节点发生的地点ID，指向Wiki中的一个地点条目'),
        keyEvents: z
            .array(z.string())
            .describe('这个情节节点的关键事件列表，包含在这个节点中发生的主要事件的描述')
    })
});
...
export const PlotNodeSchema = z.object({
    ...
    sceneTextRef: z
        .string()
        .describe('当前情节节点对应的场景文本的Hash值，指向外部存储（文件，OSS）中的一个文本块'),
    milestone: z
        .string()
        .nullable()
        .describe(
            '当前情节节点的里程碑，描述这个节点发生了什么影响后续剧情的事件，如果没有里程碑则为null'
        ),
    metadata: PlotNodeMetadataSchema.describe('当前情节节点的元数据'),
    depth: z
        .number()
        .describe('当前情节节点在情节树中的深度，根节点的深度为0，子节点的深度为父节点的深度加1'),
